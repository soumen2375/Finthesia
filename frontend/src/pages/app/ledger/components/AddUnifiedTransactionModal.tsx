...
import { X, BookOpen, Handshake, Calendar, FileText, UserSquare2, RefreshCcw, Tag, Paperclip } from 'lucide-react';
  ...
  return (
    <AnimatePresence>
      ...
      <motion.div initial={{y:'100%', opacity: 0}} animate={{y:0, opacity: 1}} exit={{y:'100%', opacity: 0}}
        ...
        className="fixed inset-x-0 bottom-0 md:inset-auto md:top-1/2 md:left-1/2 md:-translate-x-1/2 md:-translate-y-1/2 z-[101] bg-card md:rounded-[2rem] shadow-2xl overflow-hidden w-full max-w-2xl flex flex-col md:max-h-[90vh]"
        style={{ margin: 0 }}
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-5 md:p-8 space-y-6 bg-background">
           ...
           <div className="text-center py-4">
              {/* Type Toggle */}
              <div className="inline-flex bg-card p-1 rounded-full border border-border mb-4 shadow-sm">
                 {mode === 'cash' ? (
                    <>
                     <button onClick={() => setCashType('cash_in')} className={cn("px-6 py-2.5 rounded-full font-bold text-xs uppercase tracking-wider transition-all", cashType === 'cash_in' ? "bg-emerald-500 text-white shadow-sm" : "hover:bg-background text-text-muted")}>Cash In</button>
                     <button onClick={() => setCashType('cash_out')} className={cn("px-6 py-2.5 rounded-full font-bold text-xs uppercase tracking-wider transition-all", cashType === 'cash_out' ? "bg-danger text-white shadow-sm" : "hover:bg-background text-text-muted")}>Cash Out</button>
                    </>
                 ) : (
                    <>
                     <button onClick={() => setPartyType('got')} className={cn("px-6 py-2.5 rounded-full font-bold text-xs uppercase tracking-wider transition-all", partyType === 'got' ? "bg-emerald-500 text-white shadow-sm" : "hover:bg-background text-text-muted")}>You Got</button>
                     <button onClick={() => setPartyType('gave')} className={cn("px-6 py-2.5 rounded-full font-bold text-xs uppercase tracking-wider transition-all", partyType === 'gave' ? "bg-danger text-white shadow-sm" : "hover:bg-background text-text-muted")}>You Gave</button>
                    </>
                 )}
              </div>

              {/* Amount input — direct input, no calculator */}
              <div className="flex items-center justify-center gap-2">
                 <span className={cn("text-3xl md:text-4xl font-headline font-bold", activeColor)}>₹</span>
                 <input 
                   type="text" 
                   inputMode="decimal"
                   value={amount} 
                   ...
                   placeholder="0"
                   className={cn(
                     "bg-transparent border-none text-5xl md:text-6xl font-headline font-extrabold tracking-tighter text-center max-w-[300px] focus:outline-none focus:ring-0",
