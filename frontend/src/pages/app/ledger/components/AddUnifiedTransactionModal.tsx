    ...
    <AnimatePresence>
      ...
      >
        ...
        <div className="flex items-center justify-between px-8 py-5 border-b border-border bg-card">
           <h2 className="text-xl font-bold font-headline text-text-dark tracking-tight">New Transaction</h2>
           <button onClick={onClose} className="p-2 rounded-xl bg-background border border-border hover:bg-slate-50 text-text-muted transition-colors shadow-sm"><X size={20}/></button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 flex-1 overflow-y-auto w-full max-w-full m-0 p-0 overflow-x-hidden bg-background">
           ...
           <div className="col-span-1 md:col-span-8 p-8 space-y-8 overflow-y-auto">
              ...
              <div className="text-center py-6">
                 {/* Type Toggle */}
                 <div className="inline-flex bg-card p-1 rounded-full border border-border mb-6 shadow-sm">
                    {mode === 'cash' ? (
                       <>
                        <button onClick={() => setCashType('cash_in')} className={cn("px-8 py-2.5 rounded-full font-bold text-xs uppercase tracking-wider transition-all", cashType === 'cash_in' ? "bg-emerald-500 text-white shadow-sm" : "hover:bg-background text-text-muted")}>Cash In</button>
                        <button onClick={() => setCashType('cash_out')} className={cn("px-8 py-2.5 rounded-full font-bold text-xs uppercase tracking-wider transition-all", cashType === 'cash_out' ? "bg-red-500 text-white shadow-sm" : "hover:bg-background text-text-muted")}>Cash Out</button>
                       </>
                    ) : (
                       <>
                        <button onClick={() => setPartyType('got')} className={cn("px-8 py-2.5 rounded-full font-bold text-xs uppercase tracking-wider transition-all", partyType === 'got' ? "bg-emerald-500 text-white shadow-sm" : "hover:bg-background text-text-muted")}>You Got</button>
                        <button onClick={() => setPartyType('gave')} className={cn("px-8 py-2.5 rounded-full font-bold text-xs uppercase tracking-wider transition-all", partyType === 'gave' ? "bg-red-500 text-white shadow-sm" : "hover:bg-background text-text-muted")}>You Gave</button>
                       </>
                    )}
                 </div>
                 ...
                 <div className="flex items-center justify-center gap-2">
                    <span className={cn("text-4xl font-headline font-bold mb-2", 
                       mode === 'cash' ? (cashType === 'cash_in' ? 'text-emerald-500' : 'text-red-500') : (partyType === 'got' ? 'text-emerald-500' : 'text-red-500') 
                    )}>₹</span>
                    <input 
                      type="text" 
                      value={amount} 
                      readOnly
                      placeholder="0"
                      className={cn("bg-transparent border-none text-7xl font-headline font-extrabold tracking-tighter text-center max-w-[300px] focus:outline-none focus:ring-0", 
                         mode === 'cash' ? (cashType === 'cash_in' ? 'text-emerald-500 placeholder:text-emerald-500/30' : 'text-red-500 placeholder:text-red-500/30') : (partyType === 'got' ? 'text-emerald-500 placeholder:text-emerald-500/30' : 'text-red-500 placeholder:text-red-500/30') 
                      )}
                    />
                 </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                     ...
                     <div className="space-y-1.5 md:col-span-1 border border-border bg-card rounded-2xl p-2 shadow-sm focus-within:ring-2 focus-within:ring-primary/20 transition-all">
                        ...
                        <div className="flex items-center">
                          ...
                          <select value={partyId} onChange={e => setPartyId(e.target.value)} className="w-full bg-transparent border-none text-sm font-bold text-text-dark px-3 py-1 focus:ring-0 cursor-pointer">
                             ...
                             {parties.map(p => <option key={p.party_id} value={p.party_id}>{p.name}</option>)}
                          </select>
                        </div>
                     </div>
                 )}
                 ...
                 <div className="space-y-1.5 md:col-span-2 border border-border bg-card rounded-2xl p-3 shadow-sm focus-within:ring-2 focus-within:ring-primary/20 transition-all h-28 flex flex-col">
              ...
              <div className="border border-dashed border-border rounded-2xl p-6 bg-card/50">
           ...
           {/* Right Panel: Keypad & CTA (4 columns) */}
           <div className="col-span-1 md:col-span-4 bg-card border-t md:border-t-0 md:border-l border-border p-6 flex flex-col justify-between">
              ...
              <div className="bg-gradient-to-br from-primary to-blue-600 rounded-2xl p-6 text-white shadow-lg mb-6 relative overflow-hidden">
                 ...
                 <p className="text-xs font-medium opacity-90 leading-tight">
                    {mode === 'cash' ? (cashType === 'cash_in' ? `Inflow for ${category}` : `Outflow for ${category}`) : (partyType === 'got' ? `Received from ${parties.find(p => p.party_id === partyId)?.name || 'Party'}` : `Given to ${parties.find(p => p.party_id === partyId)?.name || 'Party'}`)}
                 </p>
              ...
              <div className="grid grid-cols-3 gap-2 flex-grow mb-6 max-h-[300px]">
                 ...
                 <button onClick={handleBackspace} className="font-headline font-extrabold text-xl py-4 rounded-xl bg-background border border-border hover:bg-red-500/10 hover:text-red-500 transition-colors active:scale-95 shadow-sm text-text-dark flex items-center justify-center">
                    <Delete size={28} />
                 </button>
              </div>
